...
void add_key(struct env_t *env, char c) {
  env->size++;
...
void process_key(struct env_t *env, char c) {
...
void parse(char *buffer, size_t bytes, struct env_t *env) {
  ...
  const char *digits="0123456789", *quotes="'\"";
  const char *buffer_char=buffer;
